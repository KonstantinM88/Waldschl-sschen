...
export default function AdminLocaleSwitcher({
  ...
  return (
    <div
      className={[
        "inline-flex w-full min-w-0 flex-wrap items-center justify-between gap-1.5 rounded-xl border p-1.5 backdrop-blur-sm sm:w-auto sm:flex-nowrap sm:justify-start",
        light
          ? "border-[#ddd2c0] bg-[#f7f1e8] shadow-[0_10px_24px_rgba(28,21,16,0.045)]"
      ...
      {(["de", "ru"] as const).map((option) => {
        ...
        return (
          <a
            ...
            className={[
              "inline-flex h-9 min-w-[3rem] flex-1 items-center justify-center rounded-lg px-3.5 text-[0.64rem] font-semibold uppercase tracking-[0.16em] transition-all duration-300 sm:flex-none",
              active
                ? light
                  ? "border border-[#d1ac70] bg-[#e4c58f] text-[#5b4420] shadow-[0_8px_18px_rgba(128,92,39,0.14)]"
